...
* **Preconditions and Postconditions**: In order to ask a question to the Speaker, the Participant must first login , select "join meeting" and write the name of the meeting. Then, press a button on the left bottom part of the screen with a speech buble and write the message. The Speaker will be able to see afterwards.

* **Normal Flow**: 
 *The Participant logs in;
 *The Participant selects the "join meeting" option;
 *The Participant writes the name of the meeting/conference;
 *The Participant, in the meeting, presses the speech buble icon on the bottom left part of the screen;
...
* **Preconditions and Postconditions**: In order to login, the user needs to be registered.In order to register, the user needs to go to signin and register with a valid email and password. Once successful, they will be redirected automatically to login screen where they write their email and password in order to go to the main menu.
If the participant is already registered, they can go directly to the login screen by pressing the corresponding button.

* **Normal Flow**: 
 *The Participant goes to "login" and writes their email and password;
 **OR**
 *The Participant goes to "sign up" and writes a valid email and password;
 *The Participant logs in;
...
* **Actor**: Participant
* **Description**: This use case exists so that the Participant can choose topics of interest. He can do that by creating a chat room with those topics.
...
* **Preconditions and Postconditions**: In order to create a conference room, the Speaker must first login into the application and press the button to create one. There they write the conference name, subject of the room and username they want to display.

* **Normal Flow**: 
 *The Speaker logs in;
 *The Speaker selects the option to create a conference room;
 *The Speaker chooses the name of the Conference, the subject/topic and the username displayed(seen by other people who join).
